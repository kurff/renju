...
DEFINE_string(prototxt, "lenet.prototxt", "prototxt file of caffe model");
DEFINE_string(caffemodel, "lenet_iter_10000.caffemodel", "weight file of caffe model");
...
int main(int argc, char** argv){
    ...
    auto net_server = new NetServer<State>();
    net_server->init(FLAGS_prototxt, FLAGS_caffemodel);

    std::shared_ptr<NetServer<State> > service(net_server);
